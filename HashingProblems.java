 ...
 class HashingProblems {
     ...
     public double getAverage(HashMap<Integer, Integer> map, int[] array) {
        ...
        for (int i = 0; i < array.length; i++) {
            if (map.containsKey(array[i])) {
                count++;
                average += map.get(array[i]);
            }
        }
        average = average/count;

        return average;
        // if there were no common values, average would be equal to 0.0/0.0, fulfilling requirement
        // for the problem
   ...
   public ArrayList<String> odd(HashMap<Integer, String> map) {
       ...
       ArrayList<String> result = new ArrayList<>();
        ...
        // create array of keys, based on keySet method
        for (int i = 0; i < keyArray.length; i++) {
            if ((int) keyArray[i] % 2 == 1) {
                result.add(map.get(keyArray[i]));
            }
        } // check if key is odd, if so, use key to get corresponding values
          // and add these to the ArrayList
   ...
   public int twoSums(int[] numbers, int k) {
        HashSet<Integer> newSet = new HashSet<>(); // create HashSet
        int count = 0;

        for (int i = 0; i < numbers.length; i++) {
            newSet.add(numbers[i]);
        } // populate HashSet based on array, time complexity of n

        for (int i : newSet) { // iterating over hashSet
            if (newSet.contains(i + k)) {
                count++;
            }
        } // for-loop to do look-up on each iteration, has time complexity of n
