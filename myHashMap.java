 ...
 ****************************************/
 ...
 class myHashMap<K,V> {
     ...
     public V remove(K key) {
        ...
        while (head != null) {
            if (head.key.equals(key)) { 
                ...
                // if key is found, removes it from the chained list
                size--; // readjusting size
                return head.value;
            }
            previous = head;
            head = head.next;
            // check next node in chained list
        }
        return null;
        // otherwise null
     ...
     public V replace(K key, V val) {
        ...
        // iterate through hash map, other replace method already checked if value mapping
        // was correct, so just iterate through, until key is found, and replace it
        // then returns the original value that was replaced
     ...
     public boolean replace(K key, V oldVal, V newVal) {
        V originalValue = get(key);
         ...
         if (originalValue == null || 
            (! originalValue.equals(oldVal)) ) {
             return false;
         } // if the value associated with key is null,
            // or value doesn't match specified value, return false
